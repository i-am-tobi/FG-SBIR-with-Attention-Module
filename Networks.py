...
class Attention_Module(nn.Module):    ##New Implementation
    def __init__(self,in_chans):
        super(Attention_Module,self).__init__()

        self.cn1 = nn.Conv2d(in_chans,256,1)
        self.cn2 = nn.Conv2d(256,1,1)
        self.st = nn.Softmax2d()

    def forward(self,x):
        #Creating Shortcut Connections and Coarse Fusion
        x = F.relu(self.cn1(x))
        x = F.relu(self.cn2(x))
        x = self.st(x)
...
class VGG_Attention_Network(nn.Module):
    def __init__(self,hp):
        super(VGG_Attention_Network,self).__init__()
        self.feature_embed = VGG_Network()
        self.attn = Attention_Module(in_chans=512)
        self.pool_method = nn.AdaptiveMaxPool2d(1)
    def forward(self,x):
